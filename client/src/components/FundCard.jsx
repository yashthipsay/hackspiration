...
const FundCard = ({owner, title, description, target, deadline, amountCollected, image, handleClick}) => {
    ...
    <div className='sm:w-[288px] w-full rounded-[15px] bg-[#1c1c24] cursor-pointer' onClick={handleClick} >
        <img src={image} alt="fund" className='w-full h-[158px] object-cover rounded-[10px] ' />
        <div className='flex flex-col p-4'>
          ...
          <div className="flex items-center mt-[20px] gap-[12px]">
           ...
           <img src={thirdweb} alt="user" className="w-1/2 h-1/2 object-contain" /></div>   
          </div>
          <Tooltip label = {owner}>
          <p className="flex-1 font-epilogue font-normal text-[12px] text-[#808191] pt-[5px] truncate">by {owner}</p>
          </Tooltip>
        </div>
    </div>
  )
